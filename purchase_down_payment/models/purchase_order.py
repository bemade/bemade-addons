...
class PurchaseOrder(models.Model):
    ...
    def _nothing_to_invoice_error(self):
        return UserError(_(
            ...
            "product, go to the \"Purchase\" tab and modify control policy "
            "from \"On received quantities\" to \"On ordered quantities\"."
        ))

    def _deduct_payment(self, grouped=False, final=False, date=None):
        """Create the Bill associated to the PO, deducting down payments."""
        invoice_vals_list = []
        ...
        moves = self.env['account.move'].sudo().with_context(
            default_move_type='in_invoice').create(invoice_vals_list)
        return moves
