...
class TestQuotationAlternativeMigration(TransactionCase):
    ...
    def test_wizard_creation(self):
        ...
        wizard = self.env['sale.order.duplication.wizard'].with_context(
            default_original_order_id=self.original_order.id
        ).create({
    ...
    def test_duplication_all_lines(self):
        ...
        # purpose n'existe sur sale.order que si durpro_sale est installé
        if 'purpose' in new_order._fields:
    ...
    def test_error_handling(self):
        ...
        # Test avec un devis existant mais action_duplicate_order sur un wizard sans purpose
        wizard = self.env['sale.order.duplication.wizard'].with_context(
            default_original_order_id=self.original_order.id
        ).create({
            'original_order_id': self.original_order.id,
        })
        # Verify the wizard was created correctly
