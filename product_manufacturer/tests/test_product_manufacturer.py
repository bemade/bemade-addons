...
class TestProductManufacturer(BaseCommon):
    ...
    def test_04_manufacturer_form_smoke(self):
        """The manufacturer fields are present on the product.template form view
        and a value assigned through the form persists on the record."""
        with Form(self.env["product.template"]) as form:
            form.name = "Form Smoke Product"
            form.manufacturer_id = self.manufacturer_a
            form.manufacturer_pname = "Form Product A"
            form.manufacturer_pref = "FPA"
            form.manufacturer_purl = "https://www.manufacturera.com/form_product_a"
        product = form.record

        self.assertEqual(product.manufacturer_id, self.manufacturer_a)
        self.assertEqual(product.manufacturer_pname, "Form Product A")
        self.assertEqual(product.manufacturer_pref, "FPA")
        self.assertEqual(
            product.manufacturer_purl,
            "https://www.manufacturera.com/form_product_a",
        )
        # The single variant carries the same values (compute/inverse round-trip).
        self.assertEqual(
            product.product_variant_id.manufacturer_id, self.manufacturer_a
        )
        self.assertEqual(
            product.product_variant_id.manufacturer_pname, "Form Product A"
        )
