...
class ResPartner(models.Model):
    ...
    is_odoo_partner = fields.Boolean(
        string="Is Odoo Partner", default=False, tracking=True
    )
    ...
    def set_image_from_url(self, url):
        ...
        if response.status_code == 200:
            ...
            raise ValueError(
                "Could not download image: got response code {}".format(
    ...
    def get_odoo_partner(self):
        ...
        def extract_client_data(soup):
            partner_name_elem = soup.find(id="partner_name")
            odoo_name = (
                partner_name_elem.text.strip() if partner_name_elem else "Unknown"
            )
            odoo_note = soup.find("div", class_="mb-5")  # Updated class

            # Updated: image now uses itemprop="image" or class o_partner_image
            main_image = soup.find("img", itemprop="image") or soup.find(
                "img", class_="o_partner_image"
            )
            image_link = main_image["src"] if main_image else None

            exist_client = self.env["res.partner"].search(
                [("odoo_name", "=", odoo_name)]
            )
        ...
        def process_partner_relation(partner, client):
            if partner.id != client.id:
                ...
                _logger.info(
                    f"Partner {partner.name} is the same as client {client.name}"
                )
        ...
        def extract_partner_data(url):
            ...
            # Partner type is now in a badge span next to the name
            partner_type_badge = soup.find("span", class_=lambda x: x and "bg_" in x)
            if partner_type_badge:
                badge_class = [
                    c
                    for c in partner_type_badge.get("class", [])
                    if c.startswith("bg_")
                ]
                if badge_class:
                    partner_data["odoo_partner_type"] = (
                        badge_class[0].replace("bg_", "").lower()
                    )

            partner_data["odoo_name"] = partner_data["name"]
            ...
            partner_data["email"] = (
                email_div.string if email_div and email_div.string else False
            )
            partner_data["phone"] = (
                phone_div.string if phone_div and phone_div.string else False
            )
            partner_data["website"] = (
                website_div.string if website_div and website_div.string else False
            )
            ...
            if state_data:
                ...
                partner_data["state_id"] = (
                    ...
                    .search(
                        [("code", "=", state_data), ("country_id", "=", country_id)]
                    )
                    .id
                )
        ...
        # Updated: page structure changed, now use id="wrap" as container
        wrap_div = soup.find(id="wrap")
        if not wrap_div:
            _logger.error(
                "Could not find wrap div on partners page - structure may have changed"
            )
            return
        ...
        # Find all partner links - they contain '/partners/' but not 'country'
        for link in wrap_div.find_all("a", href=True):
            ...
            if href:
                ...
                # Check for partner detail links
                elif "/partners/" in href and "/country/" not in href:
                    clean_url = href.split("#", 1)[0].split("?", 1)[0]
                    if clean_url:  # Avoid empty strings
        ...
        for partner_url in partners_url:
            ...
            exist_odoo_name = self.env["res.partner"].search(
                [("odoo_name", "=", odoo_partner["odoo_name"])]
            )
            if exist_odoo_name:
                ...
                if exist_email:
                    ...
                    exist_phone = (
                        self.env["res.partner"].search(
                            [("phone", "=", odoo_partner["phone"])]
                        )
                        if "phone" in odoo_partner
                        else False
                    )
                    if exist_phone:
                        _logger.info(
                            f"Partner Phone {odoo_partner['name']} already exist"
                        )
            ...
            for client_url in clients_url:
                if "http" not in client_url:
                    exist_url = self.env["res.partner"].search(
                        [("odoo_url", "=", client_url)]
                    )
